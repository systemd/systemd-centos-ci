...
if [[ $NSPAWN_EC -eq 0 ]]; then
    for t in TEST-43-HOMED; do
        # 2) Run it under QEMU, but only if the systemd-nspawn run was successful
        export TESTDIR="/var/tmp/${t}_sanitizers-qemu"
        rm -fr "$TESTDIR"
        exectask "${t}_sanitizers-qemu" "make -C test/${t} clean setup run debug=1 && touch $TESTDIR/pass"
